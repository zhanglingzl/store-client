...
export const GUESTS = {
  '/guest': (req: MockRequest) => genData(req.queryString),
  '/guest/:id': (req: MockRequest) => list.find(w => w.id === +req.params.id),
  'POST /_guest/:id': (req: MockRequest) => saveData(+req.params.id, req.body),
  // 支持值为 Object 和 Array
